...
//%%%%%%%%%%%%%%%%%%%%%%%%%%    seleccion de categorias     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	if (isset($_POST['editarcat'])) {
		...
		$CONSULTAcat = $CONEXION->query("SELECT * FROM productoscat WHERE id = $id");
		$row_CONSULTAcat = $CONSULTAcat -> fetch_assoc();

		$imagen = $row_CONSULTAcat['imagen'];
		$nombre = $row_CONSULTAcat['txt'];

		$CONSULTAcatsuc = $CONEXION->query("SELECT * FROM sucursalescat WHERE sucursal = $idsuc AND nombre = '$nombre'");

		$cat_exis = $CONSULTAcatsuc ->num_rows;
;
		//conpruebo se existe ya en la tabla
		 if ($cat_exis == 0) {
		 	$sql = "INSERT INTO sucursalescat (imagen,sucursal,estatus,nombre) VALUES ('$imagen',$idsuc,1,'$nombre')";
			...
			if($insertar = $CONEXION->query($sql)){
		 	...
		 	//si exite cambia el estatus
		 		$sqlupdate = "UPDATE sucursalescat SET estatus = $activo, imagen = '$imagen' WHERE sucursal = $idsuc AND nombre = '$nombre'";

	 		if($actualizar = $CONEXION->query($sqlupdate)){
				$mensajeClase='success';
				$mensajeIcon='check';
				$mensaje='Guardado';
			}else{
